...
public class ContaInvestimento extends Conta {
    ...
    //
    // >>> EXCEÇÃO (GABARITO do Exercício 1 de tratamento de exceções): antes, um
    // percentual <= 0 caía direto em super.depositar(), que ignora valor <= 0 em
    // silêncio -- ninguém era avisado que o rendimento não aconteceu. A validação
    // abaixo troca esse silêncio por um throw explícito.
    public void aplicarRendimento(double percentual) {

        // A validação é AQUI, e não em Conta.depositar(): depositar() é usado por todo
        // depósito comum do projeto, e "percentual precisa ser positivo" só é uma regra
        // de rendimento. Por isso mesmo o throw vem ANTES do super.depositar() -- se
        // viesse depois, o rendimento inválido já teria sido processado.
        if (percentual <= 0) {
