...
class ContaInvestimentoTest {
    ...
    // GABARITO do Exercício 1 de tratamento de exceções.
    //
    // Antes, percentual <= 0 era engolido em silêncio por Conta.depositar(). Agora lança,
    // e o saldo tem que continuar exatamente onde estava -- o throw acontece ANTES do
    // super.depositar(), então nenhuma linha chega a entrar no extrato.
    @Test
    @DisplayName("percentual de rendimento negativo ou zero lança exceção e não muda o saldo")
    void percentualInvalidoLancaExcecao() {
        ContaInvestimento conta = new ContaInvestimento("Natan", "7777-7");
        conta.depositar(1000);
        ...
        assertEquals("Percentual de rendimento precisa ser positivo", erro.getMessage());
        assertThrows(IllegalArgumentException.class, () -> conta.aplicarRendimento(0));
        assertEquals(1000.0, conta.getSaldo(), DELTA);
    }
}
